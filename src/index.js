...
function decode(expr) {
    return expr.match(/.{1,10}/g).map(str => str.replace(/^0*/, '')).map(str => {
        return str.match(/.{1,2}/g).map(element => {
            if (element == "10") { element = "." }
            if (element == "11") { element = "-" }
            return element;
        }).join("");
    }).map(str => {
        if (str == "**********") { str = " " }
    ...
    // OLD LOGIC leave it just to remember what is going on with logging
